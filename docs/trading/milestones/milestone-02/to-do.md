...
- [x] Define the quarantine-first extraction workflow in docs.
- [x] Implement a checksum verification workflow and record expected SHA-256 values once available.
...
- [x] Ensure the workflow does not auto-delete the original archive before validation completes.

## Exit Criteria

- [x] No one needs to run `make setup` blindly.
- [x] Historical data can be acquired with a repeatable verification path.
- [x] The repo clearly separates trusted code from untrusted bulk data.
