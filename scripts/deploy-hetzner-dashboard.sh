...
PAPER_CATEGORY="${PAPER_CATEGORY:-tennis}"
PAPER_MAX_RESULTS="${PAPER_MAX_RESULTS:-50}"
...
create_or_update_env() {
  ...
  install -d -m 0750 -o root -g "$APP_GROUP" "$APP_DIR/runtime/betfair/certs"
...
BETFAIR_CERT_FILE=${BETFAIR_CERT_FILE:-$APP_DIR/runtime/betfair/certs/client.crt}
BETFAIR_KEY_FILE=${BETFAIR_KEY_FILE:-$APP_DIR/runtime/betfair/certs/client.key}
...
RORY_TRADER_TENNIS_MIN_HOURS_TO_EVENT=${RORY_TRADER_TENNIS_MIN_HOURS_TO_EVENT:-0.5}
RORY_TRADER_TENNIS_MAX_HOURS_TO_EVENT=${RORY_TRADER_TENNIS_MAX_HOURS_TO_EVENT:-72}
RORY_TRADER_TENNIS_MAX_SPREAD=${RORY_TRADER_TENNIS_MAX_SPREAD:-0.2}
RORY_TRADER_TENNIS_MIN_MARKET_TOTAL_MATCHED=${RORY_TRADER_TENNIS_MIN_MARKET_TOTAL_MATCHED:-50}
RORY_TRADER_BETFAIR_TENNIS_MARKET_TYPES=${RORY_TRADER_BETFAIR_TENNIS_MARKET_TYPES:-MATCH_ODDS,SET_WINNER}
RORY_TRADER_BETFAIR_TENNIS_MIN_START_MINUTES=${RORY_TRADER_BETFAIR_TENNIS_MIN_START_MINUTES:-30}
RORY_TRADER_BETFAIR_TENNIS_MAX_START_HOURS=${RORY_TRADER_BETFAIR_TENNIS_MAX_START_HOURS:-72}
BETFAIR_MARKETS_CATEGORY=${BETFAIR_MARKETS_CATEGORY:-tennis}
BETFAIR_MARKETS_MAX_RESULTS=${BETFAIR_MARKETS_MAX_RESULTS:-100}
EOF
  else
    ...
    if ! env_has_key "RORY_TRADER_TENNIS_MIN_HOURS_TO_EVENT"; then
      append_env_key "RORY_TRADER_TENNIS_MIN_HOURS_TO_EVENT" "0.5"
    fi
    if ! env_has_key "RORY_TRADER_TENNIS_MAX_HOURS_TO_EVENT"; then
      append_env_key "RORY_TRADER_TENNIS_MAX_HOURS_TO_EVENT" "72"
    fi
    if ! env_has_key "RORY_TRADER_TENNIS_MAX_SPREAD"; then
      append_env_key "RORY_TRADER_TENNIS_MAX_SPREAD" "0.2"
    fi
    if ! env_has_key "RORY_TRADER_TENNIS_MIN_MARKET_TOTAL_MATCHED"; then
      append_env_key "RORY_TRADER_TENNIS_MIN_MARKET_TOTAL_MATCHED" "50"
    fi
    if ! env_has_key "RORY_TRADER_BETFAIR_TENNIS_MARKET_TYPES"; then
      append_env_key "RORY_TRADER_BETFAIR_TENNIS_MARKET_TYPES" "MATCH_ODDS,SET_WINNER"
    fi
    if ! env_has_key "RORY_TRADER_BETFAIR_TENNIS_MIN_START_MINUTES"; then
      append_env_key "RORY_TRADER_BETFAIR_TENNIS_MIN_START_MINUTES" "30"
    fi
    if ! env_has_key "RORY_TRADER_BETFAIR_TENNIS_MAX_START_HOURS"; then
      append_env_key "RORY_TRADER_BETFAIR_TENNIS_MAX_START_HOURS" "72"
    fi
    if ! env_has_key "BETFAIR_MARKETS_CATEGORY"; then
      append_env_key "BETFAIR_MARKETS_CATEGORY" "tennis"
    fi
    if ! env_has_key "BETFAIR_MARKETS_MAX_RESULTS"; then
      append_env_key "BETFAIR_MARKETS_MAX_RESULTS" "100"
    fi
  fi
